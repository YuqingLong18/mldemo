    ...
    if (code) {
        ...
        return (
            <div className="fixed bottom-4 right-4 bg-white/90 backdrop-blur border border-indigo-100 shadow-lg p-3 rounded-full flex items-center gap-2 text-sm text-indigo-900 z-50">
                ...
                {t('student.connected')} <span className="font-mono font-bold">{code}</span>
            </div>
        );
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 animate-in fade-in zoom-in duration-200">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-slate-900">{t('student.join_modal_title')}</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">{t('student.code')}</label>
                        <input
                            type="text"
                            maxLength={6}
                            value={roomCode}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">{t('student.name')}</label>
                        <input
                            type="text"
                            value={name}
                    ...
                    >
                        {!isConnected ? <Loader2 className="w-4 h-4 animate-spin" /> : t('student.join_short')}
                    </button>
                </form>
            </div>
        </div>
